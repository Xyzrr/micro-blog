...
<p>
	We had seven figures in revenue from the top two AI labs, smooth inroads into the next three, and clear
	signals of more demand, but we decided to wind the operation down. Since many people are starting
	RL env startups now, I wanted to share our rationale.
</p>
<aside class="context">
	<p>
	...
	<p>
		...
		some visuals of our product. At the end, we had 10 engineers building sims, and ~40 data
		annotators creating tasks.
	</p>
</aside>
<p>
...
<p>
	Investors told us from the beginning that computer use didn't seem very deep. Software is
	designed to be easy to use, clicking and typing isn't that hard. And I think they were more
	correct than we gave them credit for.
</p>
<p>
...
<p>
	I went into this space thinking cross-domain generalization was mostly a myth. Several
	researchers I spoke to said that knowledge transfer is so limited, they're skeptical that
	training on a CRM would transfer learnings even to another CRM. But over twelve months, the vibe
	shifted to "models are getting shockingly good at generalization".
</p>
<p>
	Maybe I was just talking to the wrong people at first. Which points to another problem we had:
	our understanding of the domain was largely bottlenecked by how many friends I could make at the
	labs that were willing to share useful insights. I tried my hand at this networking, but it felt
	wrong because I had little to offer the researchers in return; the asymmetric neediness wasn't a
	good foundation for friendship. I think I did more here than most others in the space, but we
	still had only sparse information on important questions like "how well does generalization
	actually work?", forcing us to make big bets on thin information.
</p>
<p>
...
<p>
	...
	framed the problem as, "how do we simulate the internet with as much fidelity as possible?".
</p>
...
<ul>
	...
	<li>
		How should cross-site interactions work—is it important that if you book a flight in Expedia
		you receive an email about it in Gmail?
	</li>
...
<p>
	...
	and it gradually dawned on us how crippling that was, because these decisions are fairly
	committal and migrating later is much harder than starting from scratch.
</p>
<p>
...
<p>
	Example: say you want to train models to be really good at creating PowerPoint presentations.
	This is a deep and economically valuable task.
</p>
<p>
...
<p>
	This very quickly gets you thousands of tasks with a great difficulty distribution. There are still
	open questions like should you create a replica or just use open source, if replica then what
	features to implement, how precise should the spec be, how strict the grader should be on visual
	differences. But these questions feel tractable because you know what you're optimizing for.
	Moreover, it's not as committal. You can assign the PowerPoint experiment to one engineer, and
	they can make their decisions without affecting any of your other experiments. This is unlike the
	"simulate the internet" problem, where the infra decisions you make affect everyone.
</p>
<p>
...
<p>
	The first method maps extremely well to how bug-finding will be done in real software
	development, but there could be flaws in generating the precise spec. The second method is almost
	bulletproof, but it's slightly less realistic. (We actually used the second method to pre-screen
	our human annotators, and it was remarkably predictive of who was generally good at using
	computers. So we're confident it would work for training agents too.)
</p>
<h2>Why we stopped</h2>
<p>
	Around October we tried pivoting to the new approach. But a couple months in, we realized we were
	struggling. The personas we hired and the infra we had built were all around "simulate the
	internet". Doing the new way right would have meant laying off half the team and throwing out
	most of what we'd built.
</p>
<p>
...
<p>We considered pivoting to coding RL environments. That's an infinitely deep and economically valuable problem.</p>
<p>
...
<p>
	We felt we needed some time to rebuild our world models, cleanse our stale assumptions, figure
	out where we'd really fit best in the future. And we didn't feel we could do this while running a
	company. So we stopped.
</p>
<p>
	If you're looking to hire remote engineers or data annotators with AI experience, please let me
	know. We've hired some really great people, and they're looking for new homes.
</p>
