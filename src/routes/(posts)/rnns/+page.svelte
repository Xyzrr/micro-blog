...
<p>
	...
	what approach you take, it probably involves starting with a for loop that iterates over every
	token in the input, building up some state as it iterates. At each token, maybe you’d check
	whether it’s a noun/verb/etc, and route your logic from there.
</p>
<p>
	You certainly wouldn’t first route your logic based on i, the index of the current token. While a
	token’s location inside the sentence matters, it isn’t the principal axis from which you’d decide
	how to handle the token. Yet when you use the sliding window algorithm with an MLP, you’re
	essentially forcing i to be the routing key. Each token enters a different entry point, with a
	different set of weights, based solely on i .
</p>
<p>
	With an RNN, every token is given the same entry point, with the same set of weights. This allows
	the RNN to “reuse code” however it wants.
</p>
<p>
	An RNN’s job is this: given the latest token and a note you’ve written to yourself about the
	previous tokens you’ve seen, output the next token, and write a note to your future self to help
	with future tokens.
</p>
<p>
	You can see how this enables the RNN to handle tokens in a sentence more naturally than MLPs can.
	The RNN can say, “I just received a noun, and my notes say I last received an adjective, so the
	adjective is probably describing the noun, so let me modify my note about the noun for future
	reference”. No matter what the index is inside the sentence, the RNN can reuse its “adjective
	followed by noun” codepath. The MLP would have to bend over backwards to accomplish the same
	thing.
</p>
...
<p>
	A single RNN <em>can</em> do language translation. You’d have a special input token that means “now
	translate everything you’ve seen to Chinese”, and when the model sees that in its notes, it’ll remember
	to switch to Chinese mode. But this probably doesn’t work very well, given that no one’s ever published
	a paper on it.
</p>
<p>
	It feels a little wasteful to use the same set of weights for processing the English and for
	writing the Chinese. Your model would be twice as big and only use half its abilities at any
	moment. So what you can do is have two different RNNs, an encoder for comprehending the input, and
	decoder for synthesizing the output. When the encoder finishes reading the input, it simply hands
	its note off to the decoder.
</p>
<p>
